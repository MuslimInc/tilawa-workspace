...
- [x] **T010-008**: Widget tests for `ReciterCard` semantics, 48dp favorite target, toggle tap, favorited label, non-nested open/favorite `InkWell`s (`reciter_card_test.dart`).
- [x] **T010-009**: Explicit `button: true` on open-reciter and favorite `Semantics` (`reciter_card.dart`) for FR-003 alignment.
