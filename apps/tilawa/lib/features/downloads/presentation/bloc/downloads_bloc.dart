...
class DownloadsBloc extends HydratedBloc<DownloadsEvent, DownloadsState> {
  DownloadsBloc({
    ...
    required this._deleteReciterDownloads,
    required this._clearAllDownloads,
    required this._getTotalDownloadsSize,
    required this._checkSurahDownloaded,
    required this._validateDownloadedFile,
    required this._getValidCompletedDownloads,
    required this._checkDownloadAccess,
    required this._playDownload,
    required this._playAllDownloads,
    required this._retryDownload,
    required this._getDownloadItem,
    required this._cancelDownload,
    required this._observeGlobalDownloadProgress,
    required this._getDownloadStatus,
    required this._removeFromDownloadQueue,
    ...
    on<ClearDownloadsUiNotification>(_onClearDownloadsUiNotification);
  ...
  void _notifyUi(Emitter<DownloadsState> emit, DownloadsStatus notification) {
    emit(
      state.copyWith(
        uiNotification: notification,
        uiNotificationSeq: state.uiNotificationSeq + 1,
      ),
    );
  }

  void _onClearDownloadsUiNotification(
    ClearDownloadsUiNotification event,
    Emitter<DownloadsState> emit,
  ) {
    emit(state.copyWith(uiNotification: null));
  }
      ...
      result.fold(
        (failure) {
          _notifyUi(
            ...
            DownloadsStatus.error(
              message:
                  failure.message ?? 'Failed to get valid completed downloads',
            ),
          );
        },
        (validDownloads) {
