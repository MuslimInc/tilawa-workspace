...
bool useWhiteForeground(Color backgroundColor, {double bias = 0.0}) {
  ...
  // New:
  final int v = sqrt(
    pow((backgroundColor.r * 255).round(), 2) * 0.299 +
        pow((backgroundColor.g * 255).round(), 2) * 0.587 +
        pow((backgroundColor.b * 255).round(), 2) * 0.114,
  ).round();
  return v < 130 + bias;
}
...
Color? colorFromHex(String inputString, {bool enableAlpha = true}) {
  ...
  return enableAlpha ? color : color.withValues(alpha: 1.0);
}
...
String colorToHex(
  ...
  final String hex =
      (includeHashSign ? '#' : '') +
      (enableAlpha ? _padRadix((color.a * 255).round()) : '') +
      _padRadix((color.r * 255).round()) +
      _padRadix((color.g * 255).round()) +
      _padRadix((color.b * 255).round());
