...
void main() {
  ...
  group('status filtering', () {
    ...
    test('cancels only active downloads when list has mixed statuses', () async {
      final downloading = makeItem(id: '1', status: DownloadStatus.downloading);
      ...
      when(mockRepository.getAllDownloads()).thenAnswer(
        (_) async =>
            [downloading, pending, paused, completed, cancelled, failed],
      );
  ...
  group('reciter matching', () {
    test(
      'matches by reciter ID when reciters repository succeeds',
      () async {
    ...
    test(
      'does NOT cancel downloads belonging to other reciters',
      () async {
        final targetItem = makeItem(
          id: '1',
          status: DownloadStatus.downloading,
        );
    ...
    test(
      'cancels multiple active downloads for the same reciter',
      () async {
  ...
  group('error handling', () {
    test(
      'returns Left(ServerFailure) when getAllDownloads throws',
      () async {
        when(
          mockRepository.getAllDownloads(),
    ...
    test(
      'returns Left(ServerFailure) when cancelDownload throws',
      () async {
        final item = makeItem(id: '1', status: DownloadStatus.downloading);
        when(
          mockRepository.getAllDownloads(),
    ...
    test(
      'cancels downloads attempted before a mid-loop cancelDownload failure '
      'and stops on the throwing item',
      () async {
        final item1 = makeItem(id: '1', status: DownloadStatus.downloading);
