...
void main() async {
  try {
    ...
    firstVerse.sort((a, b) => int.parse(a['word'].toString()).compareTo(int.parse(b['word'].toString())));
    ...
    for (int s = 1; s <= 114; s++) {
      for (int a = 1; a <= 286; a++) {
        ...
        if (verses.containsKey(k)) {
          final lst = verses[k]!;
          lst.sort((a, b) => int.parse(a['word'].toString()).compareTo(int.parse(b['word'].toString())));
    ...
    await File('../../packages/quran/lib/src/data/qcf_v4_data.dart').writeAsString(sb.toString());
