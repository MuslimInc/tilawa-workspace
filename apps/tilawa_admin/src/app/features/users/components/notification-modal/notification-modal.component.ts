    ...
    @if (isOpen) {
      ...
      >
        <div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
          ...
          <div
            class="relative w-full max-w-lg transform overflow-hidden rounded-[var(--tilawa-radius-xl)] border border-[var(--tilawa-outline-variant)] bg-[var(--tilawa-surface)] px-4 pt-5 pb-4 text-left shadow-[var(--tilawa-shadow-card)] transition-all sm:my-8 sm:p-6"
          >
            <div class="text-center">
              <div
                class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[var(--tilawa-surface-high)]"
              >
                <svg
                  class="h-6 w-6 text-[var(--tilawa-primary)]"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  aria-hidden="true"
              ...
              <div class="mt-3 sm:mt-5">
                ...
                <p class="mt-2 text-sm text-[var(--tilawa-on-surface-variant)]">
                  ...
                  <span class="font-semibold text-[var(--tilawa-on-surface)]">{{
                    targetSummary
                  }}</span
                  >.
                </p>
            ...
            <div class="mt-5 space-y-4">
              <div>
                <label for="title" class="tilawa-detail-label">{{
                  'notifications_notificationTitle' | t
                }}</label>
                <input
                  type="text"
                  id="title"
              ...
              <div>
                <label for="action" class="tilawa-detail-label">{{
                  'notifications_deepLinkAction' | t
                }}</label>
                <select
                  id="action"
                  [(ngModel)]="actionType"
              ...
              @if (actionType === 'reciter' || actionType === 'quran') {
                <div>
                  <label for="actionData" class="tilawa-detail-label">
                    {{
                      actionType === 'reciter'
                        ? ('notifications_reciterId' | t)
                        : ('notifications_surahNumber' | t)
                    }}
                  </label>
                  <input
                    type="text"
                    ...
                    [placeholder]="
                      actionType === 'reciter'
                        ? ('notifications_reciterIdPlaceholder' | t)
                        : ('notifications_surahNumberPlaceholder' | t)
                    "
                    [disabled]="loading"
                  />
                </div>
              ...
              <div>
                <label for="body" class="tilawa-detail-label">{{
                  'notifications_messageBody' | t
                }}</label>
                <textarea
                  id="body"
                  rows="3"
...
export class NotificationModalComponent implements OnChanges {
  ...
  get canSend(): boolean {
    ...
    if ((this.actionType === 'reciter' || this.actionType === 'quran') && !this.actionData.trim()) {
      return false;
    }
    return true;
  }
