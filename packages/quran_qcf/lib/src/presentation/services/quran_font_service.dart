...
class QuranFontService extends ChangeNotifier {
  /// Creates a [QuranFontService] with an injected [MushafService] and [IdleScheduler].
  QuranFontService({
    required this.mushafService,
    required this.idleScheduler,
    ...
    Future<void> Function(String family, Uint8List bytes)? fontRegistrar,
       ...
       _fontZipUrlProvider =
           fontZipUrlProvider ?? (() => QuranQcfFontsConstants.fontZipUrl),
       _fontRegistrar = fontRegistrar ?? _defaultFontRegistrar;

  /// The Quran data service used to access page/word metadata.
  final QuranMushafService mushafService;

  /// Background task scheduler for GPU-heavy operations.
  final IdleScheduler idleScheduler;

  final Future<Directory> Function() _documentsDirectoryProvider;
  final String Function() _fontZipUrlProvider;
  final Future<void> Function(String family, Uint8List bytes) _fontRegistrar;
  ...
  final Map<String, Future<void>> _inFlightFontFamilies =
      <String, Future<void>>{};
  Future<void>? _inFlightFontsDownload;
  ...
  Future<void> ensureQuranDataLoaded() async {
    final bool wasLoaded = mushafService.isLoaded;
    ...
    if (!wasLoaded && mushafService.isLoaded) {
      _scheduleNotify();
    }
  }

  bool get isQuranDataLoaded => mushafService.isLoaded;
  ...
  Future<void> ensureSingleFontLoaded(int pageNumber) async {
    try {
      ...
      logger.i('[FONT] ensureSingleFontLoaded: $family');
      Map<String, File> fontFilesByFamily = await _getFontFilesByFamily();
      File? file = fontFilesByFamily[family];

      // Self-heal after app reinstall/data clear: fonts directory may be empty.
      // In that case, download the bundle and reindex once, then retry load.
      if (file == null) {
        await _ensureFontsDownloadedOnce(requestedFamily: family);

        _invalidateFontIndexCache();
        fontFilesByFamily = await _getFontFilesByFamily();
      ...
      if (file != null) {
        ...
        logger.e(
          '[FONT_ERR] Could not resolve file for family $family after reindex.',
        );
      }
    } catch (e, s) {
      logger.e(
        '[FONT_ERR] Failed to load font for page $pageNumber',
        error: e,
        stackTrace: s,
      );
      rethrow;
  ...
  Future<void> _ensureFontsDownloadedOnce({
    required String requestedFamily,
  }) async {
    if (await areFontsDownloaded()) return;

    if (_inFlightFontsDownload == null) {
      logger.w(
        '[FONT] $requestedFamily missing locally; downloading Quran fonts bundle...',
      );
    }
  ...
  Future<void> downloadFonts({Function(double)? onProgress}) async {
    ...
    final Future<void>? inFlight = _inFlightFontsDownload;
    if (inFlight != null) {
      await inFlight;
      if (onProgress != null) onProgress(1.0);
      return;
    ...
    final Future<void> downloadFuture = _downloadFontsArchive(
      onProgress: onProgress,
    ).whenComplete(() => _inFlightFontsDownload = null);
    _inFlightFontsDownload = downloadFuture;
    await downloadFuture;
  }

  Future<void> _downloadFontsArchive({Function(double)? onProgress}) async {
    final String path = await _localPath;
    await Directory(path).create(recursive: true);
    final String fontZipUrl = _fontZipUrlProvider();
    final zipFile = File('$path/${fontZipUrl.split('/').last}');
  ...
  Future<void> _ensureFontFamilyLoaded({
    ...
    try {
      ...
      logger.e(
        '[FONT_LOAD_ERR] Failed to load font family $family',
        error: e,
        stackTrace: s,
      );
      rethrow;
  ...
  Future<void> _loadFontFamily({
    ...
    try {
      ...
      // 30s timeout: real Android devices (Impeller, mid-range SoCs) need
      // significantly more time than the emulator for platform font registration.
      await _fontRegistrar(family, bytes).timeout(
        const Duration(seconds: 30),
        onTimeout: () {
          throw TimeoutException('Font registration timed out after 30s');
        },
      );
      ...
      logger.e('[FONT_LOAD_ERR] $family', error: e, stackTrace: s);
      rethrow;
    }
  }

  static Future<void> _defaultFontRegistrar(
    String family,
    Uint8List bytes,
  ) async {
  ...
  Future<void> _precacheGlyphAtlas(
    ...
    final IdleTask idleTask = idleScheduler.runWhenIdle(() async {
  ...
  Future<void> _warmFromRawData(String family) async {
    ...
    final List<List<WordData>>? pageData = mushafService.getPageData(
      pageNumber,
    );
  ...
  void _invalidateFontIndexCache() {
