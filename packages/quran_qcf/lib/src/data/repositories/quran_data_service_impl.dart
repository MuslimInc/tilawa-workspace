...
import '../../domain/repositories/quran_data_service.dart';
import '../../quran_exception.dart';
import '../sources/juzs.dart';
import '../sources/page_data.dart';
import '../sources/quarters.dart';
...
class QuranDataServiceImpl implements QuranDataService {
  ...
  @override
  List<PageSurahEntry> getPageData(int pageNumber) {
    _validatePageNumber(pageNumber);
    final List<Map<String, int>> raw = pageData[pageNumber - 1];
    return raw
        .map(
          (e) => PageSurahEntry(
            surah: e['surah']!,
  ...
  int getPageNumber(int surahNumber, int verseNumber) {
    ...
    if (_pageLookupCache == null) {
      ...
      for (var pageIndex = 0; pageIndex < pageData.length; pageIndex++) {
        ...
        final List<PageSurahEntry> entries = getPageData(pageNum);
        for (final entry in entries) {
          final int surah = entry.surah;
          final int start = entry.start;
          final int end = entry.end;
          for (var v = start; v <= end; v++) {
            _pageLookupCache!['$surah:$v'] = pageNum;
          }
        }
      }
    }
