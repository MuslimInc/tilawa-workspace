...
class SkeletonizerPaintingContext extends PaintingContext {
  ...
  /// Stack of paint bounds for leaf render objects currently being painted.
  ///
  /// Used by [SkeletonizerCanvas] to determine the bounds for skeletonizing
  /// [Picture] draws (e.g. SVG images from flutter_svg).
  final List<Rect> _leafBoundsStack = [];
  ...
  void paintChild(RenderObject child, ui.Offset offset) {
    ...
    if (!isZone && child is RenderObjectWithChildMixin) {
      ...
      if (treatAaLeaf) {
        ...
        _leafBoundsStack.add(child.paintBounds.shift(offset));
        pushedLeafBounds = true;
      }
    }
    try {
...
class SkeletonizerCanvas implements Canvas {
  ...
  void drawPicture(ui.Picture picture) {
    context._didPaint = true;
    if (context._leafBoundsStack.isNotEmpty) {
      parent.drawRect(context._leafBoundsStack.last, _shaderPaint);
    } else {
  ...
  void drawRawAtlas(
    ...
    context._didPaint = true;
    if (context._leafBoundsStack.isNotEmpty) {
      final bounds = context._leafBoundsStack.last;
      parent.drawRect(bounds, _shaderPaint);
    } else {
      parent.drawRawAtlas(
        atlas,
        rstTransforms,
