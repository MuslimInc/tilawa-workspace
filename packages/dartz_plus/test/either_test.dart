...
void main() {
  group('Either', () {
    ...
    group('flatMapAsync', () {
      test('chains async Right operations', () async {
        const either = Either<String, int>.right(5);
        final Either<String, int> result = await either.flatMapAsync(
          (r) => Future.value(Either.right(r * 2)),
        );
        expect(result.isRight(), isTrue);
        expect(result.rightValue, equals(10));
      });

      test('chains Left from flatMapAsync', () async {
        const either = Either<String, int>.right(5);
        final Either<String, int> result = await either.flatMapAsync(
          (r) => Future.value(const Either.left('failed')),
        );
        expect(result.isLeft(), isTrue);
        expect(result.leftValue, equals('failed'));
      });

      test('leaves Left unchanged', () async {
        const either = Either<String, int>.left('error');
        final Either<String, int> result = await either.flatMapAsync(
          (r) => Future.value(Either.right(r * 2)),
        );
      ...
      test('accepts synchronous Either from callback', () async {
        const either = Either<String, int>.right(5);
        final Either<String, String> result = await either.flatMapAsync(
          (r) => Either.right(r.toString()),
        );
        expect(result.isRight(), isTrue);
        expect(result.rightValue, equals('5'));
