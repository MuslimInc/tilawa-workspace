...
sealed class Either<L, R> extends Equatable {
  ...
  /// Chains async [Either] operations (async flatMap/bind).
  ///
  /// If this is a [Left], returns a [Future] that completes with it unchanged.
  /// If this is a [Right], applies [f] to the value and returns the result.
  Future<Either<L, T>> flatMapAsync<T>(FutureOr<Either<L, T>> Function(R r) f) {
    return foldAsync((l) => Left(l), f);
  }
