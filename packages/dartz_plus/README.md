...
- `Future<Either<L, T>> flatMapAsync<T>(FutureOr<Either<L, T>> Function(R r) f)`: Async version of `flatMap` for chaining async `Either` operations. If this is a `Left`, returns a completed `Future` with it unchanged
