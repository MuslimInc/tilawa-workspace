...
class InAppUpdate {
  ...
  ///
  /// Use this for optional updates so users see Play's optimized download
  /// size rather than the full APK size shown by [performImmediateUpdate].
  static Future<void> openAppStoreListing() async {
    await _channel.invokeMethod('openAppStoreListing');
  }
}
...
class AppUpdateInfo {
  ...
  /// Total bytes Play reports for the pending update download.
  final int? totalBytesToDownload;

  /// Bytes already downloaded for an in-progress flexible update.
